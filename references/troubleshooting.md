...
| A file is named `未命名-<token[:8]>.md` | Every title lookup fell back: the Markdown had no `<title>`, `wiki +node-get` had no node title, and `drive +inspect` returned an empty title | It is recorded in `title_fallbacks` (alias `title_failures`); it is a warning, not a failure, and the exit code is still `0`. Re-run later to pick up the real title (the fallback file is removed automatically). The space exporter uses the same placeholder, so both entry points agree |
...
| Several CSVs for one sheet node | The workbook has more than one visible sub-sheet | Expected: `<name>.csv` for a single sub-sheet, `<name>__<sheet name>.csv` for several, named from the real `sheet_name` field. Hidden sub-sheets are skipped; `nodes[].sheets` keeps the `sheet_id -> sheet_name -> file` mapping |
| Sub-sheet CSVs are named after an opaque id such as `__0hLOFe.csv` | Pre-1.2.1 bug: the code read `title`, which `sheets +workbook-info` never returns | Upgrade to 1.2.1+, where `sheet_name` is read first, then re-run that node so the files get readable names |
| `--resume` says `未找到可用的 state.json` and downloads everything | Checkpoints are only written while `--resume` is active, so a run started without it has no `state.json` (or the options changed and the stored signature no longer matches) | Start long exports with `--resume` from the beginning. The reverse case is reported too: a checkpoint exists but `--resume` was omitted → `本次将重新处理所有节点` |
| `_INDEX.md` prints `节点产物 277 个 + 图片等资源 136 个` | 1.2.1+ reports the two groups separately | Expected: `file_count` / `total_size_bytes` are the real disk totals and include images and attachments. The exporter's own `_INDEX.md` / `_failures.md` / `_manifest.*` / `state.json` are excluded on purpose |
